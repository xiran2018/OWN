...
//JSON格式为：
//[
//{"id":货运方式（ems）的id,"timeStyle":时间的设定方式,"time":货运时间（当选择统一的方式时）,"timeMode":[{"shippingCountryIds":所选择国家的id(中间用"|"隔开);"time":承诺到达时间},{}],"shipStyle":运费的设定方式,"shipMode":[{"shippingCountryIds":所选择国家的id(中间用"|"隔开);"selectMode":自定义还是标准;"selectQwpattern":标准运费、按照重量还是按照数量;"standardFee":标准运费;"clMin":;"clMax":;"clPrice":;"clAddNum";"clAddPrice";"weightEnd0":;"weightPrice0":;"weightEnd1":;"weightInterval1";"weightPrice1";}}]}，
//]
//var shipTemplate=[];//用json格式针对每一种货运方式，存储其相应的运费信息和时间信息，在jsp页面中设置
...
//var editShipTemplateId; //正在编辑的模板的id; 在jsp页面中设置
...
//			//console.log(htmldata);
			...
			//根据服务器返回的shipTemplate信息，把选择网页中相应物流方式的运费和时间的设置方式
			createShipInfoInPage();
//		//********************************************************
...
/**
 * 根据取出来的货运模板信息，在页面中生成相应的元素
 */
function createShipInfoInPage()
{
	$("#shipTemplateName").val(templateName);
	
	var flag=0;//标识是否有元素被选择，如果有元素被设置，那么全选按钮应该选中
	var len=shipTemplate.length;
	for(var i=0;i<len;i++)
	{
		var tempShipTemplateEle=shipTemplate[i];
		var tempshipMode=tempShipTemplateEle.shipMode;
		var tempshipModeLen=tempshipMode.length;
		
		var timeStyle=tempShipTemplateEle.timeStyle;
		var time=tempShipTemplateEle.time;
		
		if((timeStyle=="0"&&time!="0"&&time!="")||(timeStyle=="1"))
		{//说明设置了时间和运费信息
			var tempShipId=tempShipTemplateEle.id;
			
			$(":checkbox[keyid="+tempShipId+"]").attr("checked",true);
			
			var tempShipStyle=tempShipTemplateEle.shipStyle;
			
			var feeRadioName=tempShipId+"cost";
			
			$("input:radio[name="+feeRadioName+"]").prop("disabled",false);
			$("input:radio[name="+feeRadioName+"][value="+tempShipStyle+"]").prop("checked",true);
			
			var timeInput=tempShipId+"_arrivetime";
			
			var tempTimeStyle=tempShipTemplateEle.timeStyle;
			var tempTime=tempShipTemplateEle.time;
			
			
			var timeRadioName=tempShipId+"time";
			$("input:radio[name="+timeRadioName+"]").prop("disabled",false);
			$("input:radio[name="+timeRadioName+"][value="+tempTimeStyle+"]").prop("checked",true);
			if(tempTimeStyle==0)
			{
				$("#"+timeInput).prop("disabled",false);
				$("#"+timeInput).val(tempTime);
			}
...
//	alert(dataRows);
	...
	html+="<tr><td colspan='4' style='text-align: center;'><input type='button' onclick='javascript:modifyAllShipTemplate()' value='保存' style='width: 60px;height: 26px;'></td></tr>";
	html+="</table>";
	return html;
}
 ...
 * 保存模板
 */
function modifyAllShipTemplate()
{
	...
	var actionUrl="modifyShipTemplate.action";
	var tempshipTemplate=JSON.stringify(shipTemplate);
	var params={
			"id":editShipTemplateId,//模板的id，在jsp页面中设置
...
function checkShipTemplateDateIsCorrect()
{
	...
	for(var i=0;i<shipTemplateLen;i++)
	{
		...
		{//说明选择的是自定义运费
			...
			if(shipMode==undefined||tempObj==null || shipMode.length==0)
			{
				var shipName=$(":checkbox[keyid="+tempId+"]").attr("name");
		...
		{//说明是统一运输时间
			...
			if(time=="0")
			{
				...
				var shipName1=$(":checkbox[keyid="+tempId+"]").attr("name");
				var des1=shipName1+"定义的运输时间为0，请修改！";
		...
		{//说明选择的是自定义运输时间
			...
			if(timeMode==undefined||timeMode==null|| timeMode.length==0)
			{
				var shipName=$(":checkbox[keyid="+tempId+"]").attr("name");
...
{
	var tempShipTemplate=shipTemplate;
	shipTemplate=[];//清空数组，然后加入实际选中的物流方式信息到这个数组中
	var shipTemplateLen=tempShipTemplate.length;
	for(var i=0;i<shipTemplateLen;i++)
	{
		var tempObj=tempShipTemplate[i];
		var tempId=tempObj.id;
		if(contains(tempId,selectShipArray))
		{
			shipTemplate.push(tempShipTemplate[i]);
		}
	}
...
{//当选择一个全选按钮时，把全部选择按钮选中
	...
	{//说明先前没有选中
		...
		//默认选中第一个
		$("input:radio[name="+feeRadioName+"][value=0]").prop("checked",true);
		$("input:radio[name="+timeRadioName+"][value=0]").prop("checked",true);
	}
	else
	{//说明先前选中了
...
function shipTimeOperate(id)
{
	...
	html+="<input type='radio'  name='"+id+"time"+"' onchange='openTimeWindowRadio(this);' value=0 keyid='"+id+"' disabled>承诺运达时间<input type='text' keyid='"+id+"' id='"+id+"_arrivetime"+"' class='arrivetimeInput' onblur='setArriveTime(this)' name='arrivetime' maxlength='22' value='0' style='width: 36;' disabled='true'>天";
...
/**
 * 当输入统一时间的文本框，失去焦点时出发的动作，设置统一的时间为多少
 * @returns
 */
function setArriveTime(element)
{
	var keyid=element.getAttribute("keyid");  //说明操作的是哪一个货运方式的模板内容
...
function openTimeWindowRadio(element)
{
	...
	{//说明是所有的国家都是同一个时间，需要删除以前的这个对象中的timeMode变量
		//把统一设置的那个文本框设为不可用
		var inputString=keyid+"_arrivetime";
...
function generateContent(shipIdArgs,tempShipMode)
{
	...
	for(var i=0;i<len;i++)
	{
		...
		{//说明选择的是标准运费
			...
			var standardFee=tempObj.standardFee;//标准运费
		...
		{//说明选择的是自定义运费
			var qwFlag=tempObj.selectQwpattern;  //按照重量，还是按照数量选择
			if(qwFlag==1)
			{//说明选择的是按照数量
...
function handleXiangXiInfo()
{
	...
	{//说明选择的是自定义运费
		...
		{//说明选择的是按照数量
			...
			obj.shippingCountryIds=shippingCountryIds;
			obj.selectMode=flag;
			obj.selectQwpattern=qwFlag;
			
			//var tempquanaityInfo={};
			obj.clMin=cl_min;
			obj.clMax=cl_max;
			obj.clPrice=cl_price;
			obj.clAddNum=cl_add_num;
			obj.clAddPrice=cl_add_price;
		...
		{//说明选择的是按照重量
			...
			//var tempweightInfo={};
			obj.weightEnd0=weight_end0;
			obj.weightPrice0=weight_price0;
			obj.weightEnd1=weight_end1;
			obj.weightInterval1=weight_interval1;
			obj.weightPrice1=weight_price1;
...
function deleteTimeTemplateInThePage(element)
{
	...
	{
		 	var key=$(element).attr("key");//获取所要编辑的货运模版的key
			var splitKey=key.split(","); //用逗号分割这个key，第一项是shipId，第二项是：selectCountryIds
...
{
	var time=TimeModeElement.shippingTime;
...
function insertXiangXiInfo(ShipModeElement)
{
	...
	var flag=ShipModeElement.selectMode;
	...
	{//说明选择的是标准运费
		...
		var standardFee=ShipModeElement.standardFee;
	...
	{//说明选择的是自定义运费
		...
		var qwFlag=ShipModeElement.selectQwpattern;
		...
		{//说明选择的是按照数量
			...
			var cl_min=ShipModeElement.clMin;
			$("#cl_min").val(cl_min);
			var cl_max=ShipModeElement.clMax;
			$("#cl_max").val(cl_max);
			var cl_price=ShipModeElement.clPrice;
			$("#cl_price").val(cl_price);
			var cl_add_num=ShipModeElement.clAddNum;
			$("#cl_add_num").val(cl_add_num);
			var cl_add_price=ShipModeElement.clAddPrice;
		...
		{//说明选择的是按照重量
			...
			var weight_end0=ShipModeElement.weightEnd0;
			$("#weight-end-0").val(weight_end0);
			var weight_price0=ShipModeElement.weightPrice0;
			$("#weight-price-0").val(weight_price0);
			var weight_end1=ShipModeElement.weightEnd1;
			$("#weight-end-1").val(weight_end1);
			var weight_interval1=ShipModeElement.weightInterval1;
			$("#weight-interval-1").val(weight_interval1);
			var weight_price1=ShipModeElement.weightPrice1;
