...
<html>
	...
	<!--	
	    ...
	    <!-- //加载模板信息，以便在shiptemplatemange.js中使用
	       /**
	       //过滤掉从数据库取出来的数据中，shipMode和timeMode 中shippingCountryIds变量为空的元素
	       */
	       function filterShipTemplate()
	       {
	    	  var len=shipTemplate.length;
	    	  for(var i=0;i<len;i++)
	    	  {
	    		  var shipTemplateEle=shipTemplate[i];
	    		  var tempShipMode;
	    	  }
	       }
	       var templateName='<s:property value="st.name"  escape="false" />';
	       
		   var editShipTemplateId='<s:property value="id"  escape="false" />';
		   var tempShipTemplate='<s:property value="returnJson"  escape="false" />';
		   //tempShipTemplate.replace("&quot;","'");
		   shipTemplate =  eval("(" + tempShipTemplate + ")");
		   //filterShipTemplate();
		   
		-->
		</script>
		<script src="jqladmin/storemange/js/shiptemplatemange.js" type="text/javascript"></script>
