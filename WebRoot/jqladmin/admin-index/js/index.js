...
function generateBasicInfo(order,uinfo)
{
	...
	html+=" </label></span>";
	//订单时间
	html+="<span class='deal-time'> Order Time: "+order.ordercreatetime+"</span> ";
	
	//购买者
	html+="<span class='seller'>"+uinfo.username;
...
function generateXiangXiInfo(order,odsvoList,uinfo)
{
	...
	for(var i=0;i<len;i++)
	{
		...
		if(i==0)
		{
			...
			html+="<span> <a class='detail-link TP_MakePoint' target='_blank' href='orderDetailForRoot.action?orderId="+order.id+"'>View Detail</a> </span>";
			html+="</td>";
			//operate
			html+="<td rowspan='"+len+"' class='operate'>";
			html+="<a href='orderDetailForRoot.action?orderId="+order.id+"' style='width: 75px;height: 15px;' class='ui-button ui-button-normal-s button_reAddToCart' button_action='Payment'  type='button'>";
			html+="调整价格</a>";
			html+="</td>";
		}
		html+="</tr>";
...
function getPageData(pageNum)
{
		...
		var actionUrl="showAllOrdersForRoot.action";
