...
public class UploadUtilAction extends ActionSupport implements ServletResponseAware 
{
	...
	private String preicon="";//在需要上传单张图片的情形中，如果提交多次，删除上一次提交的图片
	private File iconImageupload;
	private String iconImageuploadFileName;
	...
	private Integer modify_icon_self_id;//所修改的行的主键
	private String modify_preicon="";//在需要上传修改图片的情形中，该变量代表上一次上传的图片
	private File modify_iconupload; // 在需要上传修改图片的情形中，该变量代表修改上传的图片， 和JSP中input标记name同名
	private String modify_iconuploadFileName; // 上传来的文件的名字

	/**
	 * 上传商品类别图标
	 * @return
	 */
	public String uploadCategoryIcon() 
	{
...
//		System.out.println("**********fileuploadFileName************"+fileuploadFileName);
		...
		// 获取拓展名
		if (iconImageuploadFileName.lastIndexOf(".") >= 0) 
		{
			extName = iconImageuploadFileName.substring(iconImageuploadFileName
					.lastIndexOf("."));
		}
		try 
		{
			...
			{
				FileUtils.copyFile(iconImageupload, new File(filePath));//复制图片到相应的目录下
				//删除上次提交的图片
				if(!preicon.equals("")&&preicon!=null)
				{
					String tempPrefilePath = rootPath+"/" + preicon;//先前的图片保存至后台的物理路径
					tempPrefilePath = tempPrefilePath.replace("\\", "/");
					Utility.deletFile(tempPrefilePath);
				}
				out.print("<font color='red'>" + iconImageuploadFileName
...
//		System.out.println("**********fileuploadFileName************"+fileuploadFileName);
		...
		nowTimeStr = sDateFormat.format(new Date()); // 当前时间
		// 获取拓展名
		if (modify_iconuploadFileName.lastIndexOf(".") >= 0)
		{
			extName = modify_iconuploadFileName.substring(modify_iconuploadFileName
					.lastIndexOf("."));
		}
		try 
		{
			...
			if (UtilCommon.checkIsImage(extName)) 
			{
				FileUtils.copyFile(modify_iconupload, new File(filePath));//复制图片到相应的目录下
				//删除上次提交的图片
				if(!modify_preicon.equals("")&&modify_preicon!=null)
				{
					String tempPrefilePath = rootPath+"/" + modify_preicon;//先前的图片保存至后台的物理路径
					tempPrefilePath = tempPrefilePath.replace("\\", "/");
					Utility.deletFile(tempPrefilePath);
				}
				//修改数据库中相应的图片信息
				...
				category.setCategoryId(modify_icon_self_id);
				category.setIcon(relativePathInDB);
				boolean flag=MyBatisDAO.modifyCategoryIcon(category);
				//end of 修改内容
...
//		System.out.println("**********fileuploadFileName************"+fileuploadFileName);
	...
	public File getIconImageupload() {
		return iconImageupload;
	}
	public void setIconImageupload(File iconImageupload) {
		this.iconImageupload = iconImageupload;
	}
	public String getIconImageuploadFileName() {
		return iconImageuploadFileName;
	}
	public void setIconImageuploadFileName(String iconImageuploadFileName) {
		this.iconImageuploadFileName = iconImageuploadFileName;
	}
	public Integer getModify_icon_self_id() {
		return modify_icon_self_id;
	}
	public void setModify_icon_self_id(Integer modify_icon_self_id) {
		this.modify_icon_self_id = modify_icon_self_id;
	}
	public String getModify_preicon() {
		return modify_preicon;
	}
	public void setModify_preicon(String modify_preicon) {
		this.modify_preicon = modify_preicon;
	}
	public File getModify_iconupload() {
		return modify_iconupload;
	}
	public void setModify_iconupload(File modify_iconupload) {
		this.modify_iconupload = modify_iconupload;
	}
	public String getModify_iconuploadFileName() {
		return modify_iconuploadFileName;
	}
	public void setModify_iconuploadFileName(String modify_iconuploadFileName) {
		this.modify_iconuploadFileName = modify_iconuploadFileName;
	}
