...
public class Traverse {
	...
	public String fetchChildren(List<BrandSeries> list) 
	{
		String resultStr="[";
		HashMap<Integer, List> hashBrand=new HashMap<Integer, List>(); //按照Pid作为key，放入不同的List
		for(int i=0;i<list.size();i++)
		{
			BrandSeries tempBrand=list.get(i);
			Integer PId=tempBrand.getParentBrandId();
			...
			{
				List<BrandSeries> tempList=new ArrayList<BrandSeries>();
				tempList.add(tempBrand);
...
//			System.out.println("***************根元素执行次数************************************"+temp);
			BrandSeries tempBrand=(BrandSeries) rootList.get(i);
...
//				System.out.println("****"+field.getType()+"**********"+field.getName()+"*****"+ss);
	...
	private String getChildren(BrandSeries tempBrand0,HashMap<Integer, List> hashBrand)
	{
		String resultString="[";
		Integer id=tempBrand0.getBrandId();
